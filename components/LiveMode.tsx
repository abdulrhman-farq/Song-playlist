...
import { loadTasks, type Task } from "@/lib/tasksStorage";
import { loadTimeline, type TimelineEntry } from "@/lib/timelineStorage";
import { dueDateOf } from "@/lib/dueDate";
...
export default function LiveMode({
  ...
  const nextMomentTitles = nextMoment
    ? pickTitle(nextMoment.entry.title, nextMoment.entry.titleEn, lang)
    : null;
        ...
        <section aria-labelledby="live-next-moment">
          ...
          {nextMoment && nextMomentTitles && countdownLabel ? (
            ...
            >
              ...
              {nextMoment.entry.sectionLabel || nextMoment.entry.sectionId ? (
                <div
                  style={{
                    marginTop: "var(--space-4)",
                    paddingTop: "var(--space-3)",
                    borderTop: "0.5px solid rgba(58, 44, 32, 0.1)",
                    display: "flex",
                    alignItems: "center",
                    gap: "var(--space-2)",
                    flexWrap: "wrap",
                ...
                >
                  ...
                  >
                    {nextMoment.entry.sectionLabel ??
                      nextMoment.entry.sectionId}
                  </span>
                </div>
              ) : null}
            </div>
          ...
          {activeTasks.length > 0 ? (
            ...
            >
              {activeTasks.map((task) => {
                const titles = pickTitle(task.title, task.titleEn, lang);
